...
export const garminFit = async (
  config: SourcesConfig
): Promise<KeyedDataset> => {
  ...
  const dataSetAlignment =
    (config.secondsAligment || 0) + data.device_settings.time_offset;

  const startTimestamp =
    parseGarminTimeToSeconds(data.records[0].timestamp) + dataSetAlignment;
  const endTimestamp =
    parseGarminTimeToSeconds(data.records[data.records.length - 1].timestamp) +
    dataSetAlignment;
  ...
  data.records.forEach((val) => {
    session[parseGarminTimeToSeconds(val.timestamp) + dataSetAlignment] =
      val.heart_rate;
  });
